  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status == 200) {
        Toast.fire({
          icon: "success",
          html: "<div><strong>Berhasil</strong> <br> Membuat Akun</div>",
        });
        navigate("/login");
      }
      ...
      Toast.fire({
        icon: "info",
        html: `<div><strong>Warning</strong> <br> ${error.response.data.error}</div>`,
      });
    }
  };
  ...
  return (
    <section className="">
      <div className="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
        <h1 className="font-semibold text-[#238FDD] text-center text-5xl my-5">
          Paper Management
        </h1>
        <div className="w-full bg-[#238FDD] rounded-lg shadow md:mt-0 sm:max-w-md xl:p-0 ">
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            >
              ...
              >
                {loadingLogin == true ? "Loading..." : "Register"}
              </button>
            </form>
          </div>
        </div>
